...
\subsection{Planned Comparative Table}
Results from the four conditions will be aggregated in a single
table produced by \texttt{eval\_script.py} (Macro-F1, accuracy,
per-class AUC, confusion matrix), with one row per condition. A
template is shown in Table~\ref{tab:results} so collaborators can
fill in the numbers as each model is evaluated.

\begin{table}[h]
\centering
\caption{Comparative results template (to be filled in once all
four models have been evaluated on the test set with
\texttt{eval\_script.py}).}
\label{tab:results}
\begin{tabular}{lccc}
\hline
\textbf{Model} & \textbf{Macro-F1} & \textbf{Accuracy} & \textbf{Mean AUC} \\ \hline
KNN (PCA + Euclidean)         & --- & --- & --- \\
Decision Tree (PCA + Gini)    & --- & --- & --- \\
ResNet-18 supervised baseline & --- & --- & --- \\
ResNet-18 self-training       & --- & --- & --- \\
\hline
\end{tabular}
\end{table}
...
from \texttt{resnet18\_supervised\_baseline.pth}, completing
Table~\ref{tab:results}. Beyond the four planned conditions, we
