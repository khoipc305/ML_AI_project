...
4{,}708 unlabeled samples) and trained a first CNN supervised
baseline (ResNet-18 pretrained on ImageNet, 20 epochs, Adam,
class-weighted cross-entropy) on the 520-image labeled set. The
remaining experiments---KNN and Decision Tree baselines, and
CNN with iterative self-training---together with held-out test
evaluation across all four conditions will be reported next.
Preliminary expectations are that the self-training CNN will
outperform both classical baselines and the supervised-only CNN
under the same labeled-data budget.
\end{abstract}
...
\subsection{Planned Comparative Reporting}
Results from the four conditions will be aggregated in a single
table produced by \texttt{eval\_script.py}, with one row per model
and columns for macro-F1, accuracy, mean per-class AUC, and a
reference to the corresponding confusion matrix figure. The same
script also saves a per-model PNG figure with the confusion matrix
and one-vs-rest ROC curves, which will be included as
Figures~1--4 in the next iteration of this report once each model
has been evaluated on the test set.

\subsection{Expected Findings}
Based on related work (Section~\ref{sec:related}) and the strongly
imbalanced class distribution (Table~\ref{tab:classdist}), we
expect the following ordering on macro-F1:
KNN $\le$ Decision Tree $<$ CNN supervised $<$ CNN with self-training.
Classical baselines on PCA-reduced pixels are likely to collapse
toward the majority class \texttt{nv}, while the CNN with
self-training should benefit most from the $4{,}708$ unlabeled
images, especially on minority classes such as \texttt{vasc} and
\texttt{df}. Confirmation of this ranking, including any failure
modes (e.g., confirmation bias from incorrect pseudo-labels), is
the central empirical question of the next phase of the project.
...
single \texttt{config.py} file, which removes a common source of
error when collaborators share preprocessed datasets. In addition,
the CNN supervised baseline has been implemented and trained for
the full 20 epochs, producing the checkpoint
\texttt{resnet18\_supervised\_baseline.pth} that will serve both as
a direct comparison point and as the warm start for the
self-training iterations. The three remaining conditions (KNN,
Decision Tree, and CNN with self-training) are specified in detail
and ready to train on the same fixed splits.

\textbf{Future directions.} The next phase is to (i) evaluate the
existing supervised baseline on the test set with
\texttt{eval\_script.py}, (ii) train and evaluate the KNN and
Decision Tree baselines on PCA-reduced features, and (iii)
implement and run the iterative self-training loop warm-started
from \texttt{resnet18\_supervised\_baseline.pth}, then aggregate
all four sets of metrics into a single comparative table and the
corresponding confusion matrix and ROC figures produced by
\texttt{eval\_script.py}. Beyond the four planned conditions, we
...
the prepared CSV splits, and dataset statistics) are available at:
\begin{center}
