...
Decision Tree baseline on PCA-reduced features. On the held-out test set, 
the KNN baseline obtains $0.733$ accuracy and $0.20$ macro-F1, 
the Decision Tree obtains $0.669$ accuracy and $0.21$
...
macro-F1 ($0.21$ vs $0.23$) and accuracy ($66.9\%$ vs $71.2\%$).
The gap is small in aggregate but the per-class behaviour differs
substantially. The gap is small in aggregate but the per-class behaviour differs
substantially. Both classical baselines illustrate the textbook failure
mode of classical learners on PCA-reduced pixels under heavy class
imbalance: maximizing overall accuracy by collapsing onto
the majority class. The Decision Tree predicts class \texttt{nv} for
$888$ of $1{,}121$ test samples ($79\%$). The KNN baseline exhibits an 
even more extreme collapse; while it achieved the highest superficial 
accuracy ($73.3\%$), it did so by predicting \texttt{nv} for $1{,}006$ of 
the $1{,}121$ samples ($90\%$) and entirely failing to identify \texttt{akiec}, 
\texttt{vasc}, and \texttt{df} (zero correct predictions). 
The ResNet-18 supervised baseline retains a
...
PCA-reduced features ($0.21$ macro-F1, $66.9\%$ accuracy), a KNN 
baseline ($0.20$ macro-F1, $73.3\%$ accuracy), and a
ResNet-18 supervised CNN baseline ($0.23$ macro-F1, $71.2\%$
accuracy). All three confirm the expected pattern of majority-class
collapse onto \texttt{nv} and strongly motivate the semi-supervised stage.
The final remaining condition (CNN with self-training) is specified in 
detail and ready to train on the same fixed splits.

\textbf{Future directions.} The next phase is to (i) implement and run the iterative
